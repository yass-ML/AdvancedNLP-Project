...
We evaluated the impact of example selection strategies on the entity extraction accuracy of Llama-3-8B. Given the structural nature of NER, we hypothesized that semantic relevance plays a crucial role in preventing entity type hallucinations.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccccc}
        ...
        \midrule
        \textbf{Qwen3-8B} & 0.402 & 0.825 & N/A & +0.423 & N/A \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccccc}
        ...
        \midrule
        \textbf{Qwen3-8B} & 0.30 & 0.53 & 0.61 & +0.23 & 74.2\% \\
...
\section{Conclusion}
This study systematically investigated the "Few-Shot Dilemma" in Small Language Models, challenging the prevailing notion that simply increasing context volume guarantees better performance. Through our targeted experiments on Competitive Math classification \textbf{and Named Entity Recognition (NER)}, we established critical findings regarding the deployment of SLMs in resource-constrained environments.
...
Finally, we quantified the \textbf{Efficiency Frontier} and its limits. Our weighted F1-based global benchmark revealed that for capable 7B-parameter models (Qwen2-7B: 85.3\%, Llama-3-8B: 84.1\%) on reasoning tasks, an optimized selection of just $K=3$ examples can recover over \textbf{80\%} of the F1-weighted performance gain typically achieved through expensive supervised fine-tuning. However, our NER experiments revealed an important boundary. While optimized few-shot learning improved entity extraction capabilities (improving Llama-3 by +48.4\% over weak baselines), generalist SLMs still trailed the specialized NuNER Zero baseline by a significant margin (0.46 vs 0.61 F1). This highlights an "Evaluation Gap" for structured tasks, where the lack of specific architectural priors in generalist models cannot be fully compensated for by context optimization alone.

...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccccc}
        ...
        \midrule
        \textbf{Qwen3-8B} & 0.51 & 0.80 & [VAL] & +0.29 & [VAL]\% \\
        \textbf{Qwen2-7B} & 0.49 & 0.75 & 0.85 & +0.26 & 72.2\% \\
        \textbf{Llama-3-8B} & 0.46 & 0.76 & 0.83 & +0.30 & 81.1\% \\
        \textbf{Mistral-7B} & 0.43 & 0.70 & 0.81 & +0.27 & 71.1\% \\
        \textbf{Gemma-7B} & 0.37 & 0.68 & 0.75 & +0.31 & 81.6\% \\
        \textbf{Phi-3-Mini} & 0.33 & 0.38 & 0.67 & +0.05 & 14.7\% \\
        \bottomrule
    \end{tabular}
\end{table}

The benchmark reveals that for capable models like Llama-3 and Gemma, our optimized few-shot strategy recovers over \textbf{80\%} of the performance gains associated with full fine-tuning. This suggests that for these architectures, specialized ICL strategies offer a highly efficient alternative to expensive training pipelines. Conversely, smaller models like Phi-3-Mini show a low recovery rate (14.7\%), indicating that their limitations are structural and require parameter updates to overcome.
...
First, we demonstrated that \textbf{quality outweighs quantity}. Naive random selection provided negligible benefits over zero-shot baselines, whereas our proposed DPO-Hybrid selector, which balances semantic similarity with label correctness, achieved state-of-the-art results. This confirms that for SLMs, the \textit{relevance} of the context is a stronger predictor of success than the \textit{volume} of the context.

Second, we identified a \textbf{divergence in architectural needs}. We observed that "Reasoning Models" (like Qwen3) exhibit an early performance peak ($K=3$) and degrade rapidly with more examples, effectively suffering from "over-prompting" due to interference with their internal chain-of-thought processes. In contrast, "Heuristic Models" (like Llama-3) treat few-shot examples as statistical data points, scaling robustly up to $K=20$. This suggests that prompt engineering strategies must be tailored not just to the task, but to the specific cognitive architecture of the model.

Finally, we quantified the \textbf{Efficiency Frontier}. Our global benchmark revealed that for capable 7B-parameter models, an optimized selection of just $K=3$ examples can recover over \textbf{80\%} of the performance gain typically achieved through expensive supervised fine-tuning. This effectively identifies a "Pareto Optimal" sweet spot: by using a smart, low-latency selector, practitioners can achieve near-supervised performance without the high computational cost of training or the high inference latency of long-context prompting.

...
class MetricsPipeline:
    def __init__(self, model_name="llama3:8b", dataset_path="datasets/competition_math",
        ...
        Args:
            ...
            dataset_path: Path to the dataset folder (Evaluation Data).
            ...
            train_dataset_path: Path to the training dataset (for Shot Selection).
        """
    ...
    def _load_parquet_files(self, path):
        if not path: return None
        search_path = os.path.join(path, "**", "*.parquet")
        files = glob.glob(search_path, recursive=True)

        if not files:
            search_path = os.path.join(path, "data", "*.parquet")
            files = glob.glob(search_path, recursive=True)
    ...
    def load_data(self):
        """Loads the competition_math dataset."""
        print(f"Loading EVALUATION data from {self.dataset_path}...")
        self.df = self._load_parquet_files(self.dataset_path)
        print(f"Loaded {len(self.df)} evaluation examples.")

        if self.k_shots > 0:
            selector_df = None
            if self.train_dataset_path:
                 print(f"Loading SELECTION data from {self.train_dataset_path}...")
                 selector_df = self._load_parquet_files(self.train_dataset_path)
            else:
                 print("WARNING: No train_dataset_path provided. Using EVALUATION data for selection! (Risk of data leakage)")
                 selector_df = self.df
